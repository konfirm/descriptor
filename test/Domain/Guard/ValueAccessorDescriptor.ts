...
test('Domain/Guard/ValueAccessorDescriptor - exports', (t) => {
	const expect = ['isValueAccessorDescriptor'];
...
const { isValueAccessorDescriptor } = Export;

test('Domain/Guard/ValueAccessorDescriptor - isValueAccessorDescriptor', (t) => {
	each`
	...
	`(({ valid, ...rest }: any) => {
		...
		t.equal(isValueAccessorDescriptor(defined), match, `${stringify(defined)} is ${message}`);
	});
